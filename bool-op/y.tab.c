...
#line 19 "expr.y" /* yacc.c:355  */
  ...
  struct {
    ...
    struct quad* code;
    // ajouter un label du quad de l'expr ?
	} quadlist;
...
{
       0,    46,    46,    80,    92,   103,   107,   114,   118,   134,
     142
};
#endif
...
{
  "$end", "error", "$undefined", "OR", "AND", "NOT", "ID", "NUM", "'\\n'",
  "'>'", "'('", "')'", "'='", "$accept", "axiom", "condition", "expr", YY_NULLPTR
};
#endif
...
{
       0,   256,   257,   258,   259,   260,   261,   262,    10,    62,
      40,    41,    61
};
# endif
...
{
       7,     7,   -10,   -10,     7,     8,    12,    -5,   -10,    -2,
     -10,     7,     7,   -10,    15,    15,   -10,    14,   -10,   -10,
     -10
};
...
{
       8,    11,    12,     9,    14,    19,    20,    15,    10,    16,
      17,    18,     1,     2,     3,    11,    12,     4,    12,     0,
      13,     2,     3
};

static const yytype_int8 yycheck[] =
{
       1,     3,     4,     4,     9,    14,    15,    12,     0,    11,
      11,    12,     5,     6,     7,     3,     4,    10,     4,    -1,
       8,     6,     7
};
...
{
       0,     5,     6,     7,    10,    14,    15,    16,    15,    15,
       0,     3,     4,     8,     9,    12,    11,    15,    15,    16,
      16
};
...
#line 47 "expr.y" /* yacc.c:1646  */
    {
                              ...
                              quad_list_complete((yyvsp[-1].quadlist).truelist, label_true);
                              quad_list_complete((yyvsp[-1].quadlist).falselist, label_false);

                              code = (yyvsp[-1].quadlist).code;
...
#line 81 "expr.y" /* yacc.c:1646  */
    {
                              ...
                              struct quad* goto_true   = quad_gen(next_quad++, '>', (yyvsp[-2].codegen).result, (yyvsp[0].codegen).result, NULL); // res null -> trou
                              struct quad* goto_false  = quad_gen(next_quad++, 'G', NULL, NULL, NULL);
                              (yyval.quadlist).code = (yyvsp[-2].codegen).code;
                              quad_add(&(yyval.quadlist).code, (yyvsp[0].codegen).code);
                              quad_add(&(yyval.quadlist).code, goto_true);
                              quad_add(&(yyval.quadlist).code, goto_false);
                              (yyval.quadlist).truelist   = quad_list_new(goto_true);
                              (yyval.quadlist).falselist  = quad_list_new(goto_false);
                            }
...
#line 108 "expr.y" /* yacc.c:1646  */
    {
                              ...
                              (yyval.quadlist).code = (yyvsp[0].quadlist).code;
                              (yyval.quadlist).truelist = (yyvsp[0].quadlist).falselist;
                              (yyval.quadlist).falselist = (yyvsp[0].quadlist).truelist;
                            }
...
#line 118 "expr.y" /* yacc.c:1646  */
    {
                              ...
                              quad_add(&code, (yyvsp[0].codegen).code); // store the E code

                              if((new_id = symbol_find(tds, (yyvsp[-2].codegen).result->id)) != NULL){ // new id already existe
                                  fprintf(stderr,"error: redeclaration of with no linkage");
                                  exit(EXIT_FAILURE);
                              } else {
                                  new_id = symbol_add(tds, (yyvsp[-2].codegen).result->id);
                                  new_id->value = (int)(yyvsp[0].codegen).result->value; }

                              quad_add(&code, quad_gen(next_quad++,'=', (yyvsp[0].codegen).result,NULL, new_id)); // store this stmnt code
                            }
...
#line 135 "expr.y" /* yacc.c:1646  */
    {
                              printf("expr -> ID (%s)\n", (yyvsp[0].string));
                              (yyval.codegen).result = symbol_find(tds, (yyvsp[0].string));
...
#line 143 "expr.y" /* yacc.c:1646  */
    {
                              printf("expr -> NUM (%d)\n", (yyvsp[0].value));
                              if(tds == NULL) {
                                tds = symbol_newcst(&tds, (yyvsp[0].value));
                                (yyval.codegen).result = tds;
                              }
                              else
                                (yyval.codegen).result = symbol_newcst(&tds, (yyvsp[0].value));
                              (yyval.codegen).code=NULL;
                            }
